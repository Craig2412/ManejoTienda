...
@section('content')
    ...
    <div class="space-y-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-semibold">Pagos cuenta #{{ $order->id }}</h1>
                <p class="text-sm text-slate-500">Registra abonos a la cuenta por cobrar.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('debts.show', $order) }}" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100">
                    Volver
                </a>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
                            ...
                            @forelse ($order->payments as $payment)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        @if (! $isClosed)
                                            <form method="POST" action="{{ route('debts.payments.destroy', [$order, $payment]) }}">
                                                @csrf
                                                @method('DELETE')
            ...
            <div class="space-y-4">
                <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="text-lg font-semibold">Abono</h2>
                    @if (! $isClosed)
                        <form method="POST" action="{{ route('debts.payments.store', $order) }}" class="mt-4 space-y-4">
                            @csrf
                            ...
                            <div class="flex items-center justify-between">
                                <div class="text-sm text-slate-500">
                                    Total: <span class="font-semibold text-slate-800">${{ number_format($order->total, 2) }}</span>
                                    | Pagado: <span class="font-semibold text-emerald-700">${{ number_format($totalPaid, 2) }}</span>
                                    | Saldo: <span class="font-semibold text-red-600">${{ number_format($remaining, 2) }}</span>
                                </div>
                ...
                @if (! $isClosed)
                    <form method="POST" action="{{ route('debts.close', $order) }}">
                        @csrf
                        <button type="submit" class="w-full rounded-lg bg-amber-500 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-600" @disabled($remaining > 0)>
                            Cerrar cuenta
                        </button>
                        @if ($remaining > 0)
                            <p class="mt-2 text-xs text-slate-500">El saldo debe quedar en cero para cerrar.</p>
                        @endif
                    </form>
                @endif
            </div>
