...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-semibold">Pago de Mesa {{ $table->numero }}</h1>
                <p class="text-sm text-slate-500">Selecciona el metodo y la moneda para cerrar la orden.</p>
            </div>
            <a href="{{ route('pos.tables.show', $table) }}" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100">
                Volver
            </a>
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
                            ...
                            @forelse ($order->payments as $payment)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <form method="POST" action="{{ route('pos.tables.payments.destroy', [$table, $payment]) }}">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="space-y-4">
                <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <form method="POST" action="{{ route('pos.tables.payments.store', $table) }}" class="mt-4 space-y-4">
                        @csrf
                ...
                <form method="POST" action="{{ route('pos.tables.close', $table) }}">
                    @csrf
                    ...
                    @if ($remaining > 0)
                        <p class="mt-2 text-xs text-slate-500">Debes completar el pago total para liberar la mesa.</p>
                    @endif
                </form>

                @if ($remaining > 0)
                    <form method="POST" action="{{ route('pos.tables.debt', $table) }}" onsubmit="return confirm('Deseas anexar esta orden a cuentas por cobrar?');">
                        @csrf
